...
Настройки приложения в Perlin
-----------------------------

По-умолчанию, настройки приожения находятся в папке ```app/config``` и используется формат ```ini```, но вы можете использовать любой другой:
...
Pherlin использует конфигурационный файл для регистрации сервисов в контейнере зависимостей. Настройка сервисов должна нахоится в конфигурации приложения, в переменной ```dependencies```. Вот как это реализовано в Pherlin по-умолчанию:
